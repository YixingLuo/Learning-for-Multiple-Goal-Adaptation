function [c,ceq] = myconuav_relax3(x)
y = [0,0,0,0,0];
for i = 1:5
    if (x(end-5+1)>0.5)
        y(i) = 1;
...
c = [c, - info_now + (configure.forensic_target - y(5)*x(end-5-2))];
c = [c,time_now - (configure.Time_target + y(4)*x(end-5-1))];
c = [c, energy_now - (configure.battery_target + y(3)*x(end-5))];
...
for j = 1: length_o
    for i = 1:initial_N + 1
%         c = [c, - dis_o(i, j) + (configure.radius + configure.obstacle_radius + y(1)*x(end-5-4))];
        c = [c, - dis_o(i, j) + (configure.radius + configure.obstacle_radius + y(1)*configure.obstacle_max)];
    end
end

for j = 1: length_p
    for i = 1:initial_N + 1
%         c = [c, - dis_p(i, j) + (configure.radius + configure.privacy_radius + y(2)*x(end-5-3))];
        c = [c, - dis_p(i, j) + (configure.radius + configure.privacy_radius + y(2)*configure.privacy_max)];
    end
end
