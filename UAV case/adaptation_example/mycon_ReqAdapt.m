...
c = [c, - info_now + (configure.forensic_target - y(3)*x(end-5-2))];
c = [c,time_now - (configure.Time_target + y(4)*x(end-5-1))];
c = [c, energy_now - (configure.battery_target + y(5)*x(end-5))];
...
for j = 1: length_o
    for i = 1:initial_N + 1 
        ...
        c = [c, - dis_o(i, j) + (configure.radius + configure.obstacle_radius + configure.obstacle_max - y(1)* x(x_index))];
    end
end
